...
RbtBool bAdInversionFlag(false);
...
#if 1 // AdvanceSoft (2017)
		  if (spA1->GetAtomName() == "CA") {
			  bAdInversionFlag = true;
		  }
#endif // AdvanceSoft(2017)
...
//Returns true if bond is in a ring (i.e. if atom 1's flag gets set also)
//DM 8 Feb 2000 - standalone version (formerly only available as RbtModel method)
RbtBool Rbt::Ad2ToSpin(RbtBondPtr spBond, RbtAtomList& atomList, RbtBondList& bondList)
{
	RbtAtomPtr spAtom1 = spBond->GetAtom1Ptr();
	RbtAtomPtr spAtom2 = spBond->GetAtom2Ptr();
	...
	//Seed the pending list with atom 1
	spAtom1->SetSelectionFlag(true);
	spBond->SetSelectionFlag(true);
	pendingAtomList.push_back(spAtom1);
