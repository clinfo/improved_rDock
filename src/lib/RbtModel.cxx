...
#if 1 // Xability (2017)
static RbtCoord RotateV(const RbtCoord& vAxis, RbtDouble s, RbtDouble c,
                        const RbtCoord& v)
{
  RbtDouble nx = vAxis.x;
  RbtDouble ny = vAxis.y;
  RbtDouble nz = vAxis.z;
  RbtDouble c1 = 1.0 - c;
  RbtDouble Rxx = nx * nx * c1 + c;
  RbtDouble Rxy = nx * ny * c1 - nz * s;
  RbtDouble Rxz = nx * nz * c1 + ny * s;
  RbtDouble Ryx = ny * nx * c1 + nz * s;
  RbtDouble Ryy = ny * ny * c1 + c;
  RbtDouble Ryz = ny * nz * c1 - nx * s;
  RbtDouble Rzx = nz * nx * c1 - ny * s;
  RbtDouble Rzy = nz * ny * c1 + nx * s;
  RbtDouble Rzz = nz * nz * c1 + c;
  RbtCoord u;
  u.x = Rxx * v.x + Rxy * v.y + Rxz * v.z;
  u.y = Ryx * v.x + Ryy * v.y + Ryz * v.z;
  u.z = Rzx * v.x + Rzy * v.y + Rzz * v.z;
  return u;
}

static RbtCoord RotateV(const RbtCoord& vAxis, RbtDouble theta, const RbtCoord& v)
{
  RbtDouble s = sin(theta);
  RbtDouble c = cos(theta);
  return RotateV(vAxis, s, c, v);
}

// rotate v with the rotation matrix which rotates v1 to v2
static RbtCoord RotV1toV2(const RbtCoord& v1, const RbtCoord& v2, const RbtCoord& v)
{
  RbtCoord v12 = v1.Cross(v2);
  RbtDouble s = v12.Length();
  if( s < 1.0e-10 ) return v;
  RbtDouble c = v1.Dot(v2);
  v12 /= s;
  return RotateV(v12, s, c, v);
}


void RbtModel::AlignCoord(const RbtModel& model, const RbtIntList& viAtcMap, 
                          const RbtCoordList& coordAtc)
{
  int nAtomAtc = GetNumAtoms();
  int nBondAtc = GetNumBonds();

  int nAtom = model.GetNumAtoms();
  int nBond = model.GetNumBonds();

  RbtAtomList atomList = model.GetAtomList();
  RbtBondList bondList = model.GetBondList();

  // copy except H
  RbtCoordList coordRef(nAtom);
  RbtCoordList coordOrg(nAtom);
  for(RbtInt i = 0; i < nAtomAtc; ++i) {
    RbtInt j = viAtcMap[i];
    if( j < 0 ) continue;
    RbtCoord v = atomList[j]->GetCoords();
    m_atomList[i]->SetCoords(v);
    coordRef[j] = v;
    coordOrg[j] = coordAtc[i];
  }

  // ignored H
  RbtIntList viRoot(nAtomAtc, -1);
  for(RbtInt i = 0; i < nBondAtc; ++i) {
    RbtInt i1 = m_bondList[i]->GetAtom1Ptr()->GetAtomId() - 1;
    RbtInt i2 = m_bondList[i]->GetAtom2Ptr()->GetAtomId() - 1;
    if(viAtcMap[i1] < 0) viRoot[i1] = viAtcMap[i2];
    if(viAtcMap[i2] < 0) viRoot[i2] = viAtcMap[i1];
  }

  vector<RbtIntList> v12atom(nAtom);
  for(RbtInt i = 0; i < nBond; ++i) {
    RbtInt i1 = bondList[i]->GetAtom1Ptr()->GetAtomId() - 1;
    RbtInt i2 = bondList[i]->GetAtom2Ptr()->GetAtomId() - 1;
    v12atom[i1].push_back(i2);
    v12atom[i2].push_back(i1);
  }

  vector<RbtIntList> v13atom(nAtom);
  for(RbtInt i = 0; i < nAtom; ++i) {
    for(RbtInt j = 0; j < v12atom[i].size(); ++j) {
      RbtInt jj = v12atom[i][j];
      for(RbtInt k = 0; k < v12atom[jj].size(); ++k) {
        RbtInt kk = v12atom[jj][k];
        if(kk != i) {
          v13atom[i].push_back(kk);
        }
      }
    }
  }

  for(int i = 0; i < nAtomAtc; ++i) {
    if(-1 < viAtcMap[i]) continue;

    RbtInt iRoot = viRoot[i];
    if(iRoot < 0) continue;

    if( v12atom[iRoot].empty() ) {
      RbtCoord v = coordAtc[i] - coordOrg[iRoot];
      v += coordRef[iRoot];
      m_atomList[i]->SetCoords(v);
      continue;
    }
    ...
    RbtIntList viNeighb;
    if( v12atom[iRoot].size() == 1 ) {
      viNeighb = v13atom[iRoot];
    }
    else {
      viNeighb = v12atom[iRoot];
    }
    RbtInt nNeighb = viNeighb.size();

    RbtCoordList crdRot(nNeighb);
    RbtCoordList crdRef(nNeighb);
    for(RbtInt j = 0; j < nNeighb; ++j) {
      crdRot[j] = coordOrg[viNeighb[j]] - coordOrg[iRoot];
      crdRef[j] = coordRef[viNeighb[j]] - coordRef[iRoot];
    }

    RbtCoord vAxisRot;
    RbtCoord vAxisRef;
    if( v12atom[iRoot].size() == 1 ) {
      RbtInt j = v12atom[iRoot][0];
      vAxisRot = coordOrg[j] - coordOrg[iRoot];
      vAxisRef = coordRef[j] - coordRef[iRoot];
    }
    else{
      for(RbtInt j = 0; j < nNeighb; ++j) {
        vAxisRot += crdRot[j];
    ...
    vAxisRot = vAxisRot.Unit();
    vAxisRef = vAxisRef.Unit();
      
    // rotate crdRot so that vAxisRot aligns to vAxisRef
    for(RbtInt j = 0; j < nNeighb; ++j) {
      crdRot[j] = RotV1toV2(vAxisRot, vAxisRef, crdRot[j]);
    }
    ...
    RbtDouble sind = 0.0;
    for(RbtInt j = 0; j < nNeighb; ++j) {
      RbtCoord vRot = crdRot[j] - crdRot[j].Dot(vAxisRef) * vAxisRef;
      RbtCoord vRef = crdRef[j] - crdRef[j].Dot(vAxisRef) * vAxisRef;
      vRot = vRot.Unit();
      vRef = vRef.Unit();
      cosd += vRot.Dot(vRef);
      sind += vRot.Cross(vRef).Dot(vAxisRef);
    }
    RbtDouble rr = cosd * cosd + sind * sind;
    if( 1e-10 < rr ) {
      dihed = acos(cosd/sqrt(rr));
      if( sind < 0.0 ) {
        dihed = - dihed;
      }
    }
    ...
    RbtCoord v = coordAtc[i] - coordOrg[iRoot];
    v = RotV1toV2(vAxisRot, vAxisRef, v);
    v = RotateV(vAxisRef, dihed, v);
    v += coordRef[iRoot];
    m_atomList[i]->SetCoords(v);
  }
}
