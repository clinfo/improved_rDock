...
public:
  ...
  RbtChromElement* GetChrom_Ad() const { return m_chrom_Ad; }
#endif // AdvanceSoft(2018)


  
  //Sets the raw score from the scoring function.
  //Model coordinates are updated to reflect the current chromosome element values.
  //Scores are negated so that higher positive scores are better.
...
#if 1 // AdvanceSoft (2018)
  void SetScore_Ad(double score) { m_score = score; }
  void SetRWFitness_Ad(double RWFitness) { m_RWFitness = RWFitness; }
#endif // AdvanceSoft (2018)
...
#if 1 // AdvanceSoft (2018)
  RbtChromElement* m_chrom_Ad;
  std::vector<std::string> m_atomlist;
  std::vector<std::string> m_bondlist;
  std::vector<std::string> m_coordlist;
  int m_size_atomlist;
  int m_size_bondlist;
  int m_size_coordlist;
  std::vector<std::string> m_v_sdf;
#endif // AdvanceSoft(2018)
...
namespace Rbt {
    ...
    class isGenome_eq : public std::binary_function<RbtGenome*,RbtGenome*,RbtBool> {
    private:
